...
class RenderStatusBar:
	def __init__ (self):
		...
		#Blit the UM Logo
		self.notiscreen.blit(um_logo,(185,0))
...
def initscreen(width, height) :
	...
	if headless:
		...
		size = (pygame.display.Info().current_w, pygame.display.Info().current_h)
		print "Pygame dummy mode, framebuffer size: %d x %d" % (size[0], size[1])
		...
		if windowed:
			...
			size = (pygame.display.Info().current_w, pygame.display.Info().current_h)
			print "Pygame window mode, framebuffer size: %d x %d" % (size[0], size[1])
		else:
			...
			size = (pygame.display.Info().current_w, pygame.display.Info().current_h)
			print "Pygame fullscreen mode, framebuffer size: %d x %d" % (size[0], size[1])
	
	return screen

def initPyGame():
	global clock, screen, ScreenTimer, background, mystatusbar, mainmenu, campreviewmenu, mynotism, mynotiwait, current_w, current_h, um_logo, mymonitorscreen, mainmenucontrol, mainmenuscreen, mySimpleDialog
	...
	um_logo = pygame.image.load(os.path.join(resourcesdir, "um_logo_ori.png")).convert_alpha()
	um_logo.fill((buttonbordercolor.r, buttonbordercolor.g, buttonbordercolor.b , 0), special_flags = BLEND_RGBA_MAX) #blend logo to dark grey
	um_logo.fill((215, 215, 215 , 0), special_flags = BLEND_RGBA_MAX) #blend logo to white
	background.blit(um_logo,(185,200))
		...
		if result["CMD"] == "COMMAND":
			...
			if cmd == "PRINTSTOP":
				...
				if cfg.settings["General"]["email_on_complete"] == "True":
					...
					helpers.send_mail_async(cfg.settings["General"]["email_to"],
...
if __name__ == '__main__':
	...
	cfg.defaults = {"General": {"pics_per_second": "5", "tl_autostart": "False", "tl_autostop": "False",
								"email_on_complete": "False","lights_on_onstart": "False", "lights_off_oncomplete": "False", "email_to": "test@send.com", "email_user": "test@gmail.com", "email_pw": "***",
	...
	], debug=TornadoDebug, queue=taskQ)
