...
def initPyGame():
	...
	my_logo = pygame.image.load(os.path.join(resourcesdir, "logo_ori_sm.png")).convert_alpha()
...
def startstop_camera(cmd):
	global mjpgCam
	#try to re-attached the camera
	if cmd == "stop":
		if mjpgCam.is_running: mjpgCam.stop()
...
def mjpg_cmd(cmd):
	...
	if cmd == "start": exec_cmd=cmd_prefix+cfg.settings["Webserver"]["mjpg_start_cmd"]
	if cmd == "stop": exec_cmd=cmd_prefix+cfg.settings["Webserver"]["mjpg_stop_cmd"]
	
	print "MJPG: "+cmd+" with " +  exec_cmd + "..."
	
	try:
		x = subprocess.check_output([exec_cmd], shell=True)
		#x = subprocess.check_output(["raspi_stream_test.cmd"])
		if cmd=="start": time.sleep(float(cfg.settings["Webserver"]["mjpg_start_delay"]))
	except Exception as e:
		x= "MJPG: "+exec_cmd+" could not execute. Error: {name} ({msg}).".format(name=e.__class__.__name__,msg=e)
		print x
...
def SendGodeLog(cmd):
	...
	#sending it in serialprocess already!
	#updateSerialLog(time.strftime("%H:%M:%S")+" sent: "+cmd)

def send_testmail_async_complete(status):
	global emailstatus
	
	ok, msg = status
	emailstatus = {"ok": ok , "msg": msg}
...
def checkSerialQResults():
	...
	if not resultQ.empty():
		...
		#weblog handler...
		if result["CMD"] == "UPDATE_SERIAL_LOG":
			updateSerialLog(result["DATA"])
		...
		if result["CMD"] == "SERIAL":
			#if not printerstatus["isstreaming"]:
...
if __name__ == '__main__':
	...
	if headless == False:
		periodic = tornado.ioloop.PeriodicCallback(PyInterfaceMainLoop, 30, io_loop = tornado_main_loop)
		periodic.start()
	
	#Serial / GPIO Q Monitor
	serialscheduler = tornado.ioloop.PeriodicCallback(checkSerialQResults, 1, io_loop = tornado_main_loop)
